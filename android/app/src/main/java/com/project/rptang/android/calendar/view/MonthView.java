package com.project.rptang.android.calendar.view;
...
import com.project.rptang.android.calendar.utils.DateUtils;
import java.util.ArrayList;
import java.util.Calendar;
import java.util.List;

/**
 * Created by Stiven on 2017/2/6.
 */
...
public abstract class MonthView extends View{
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int heightSize = (int) (NUM_ROWS * baseRowSize)+1;
    ...
    /**
     * 在这个方法中调用了两个方法：canvas.save()、canvas.restore()
     * 这两个方法中间的代码不影响画布上其他地方
     * 可以这样想象：现在你有一张画，现在你想在空白地方再画上新的物品
     * 由于角度问题，你不得不平移画布，你希望不管如何移动画布，其他地方保持原始状态就好
     * 在save和restore方法之间去话就好了，其他地方就不会动
     */
...
//        rowSize = getHeight() * 1.0F / NUM_ROWS;
    ...
    public void smoothScrollBy(int dx, int dy) {
        //设置mScroller的滚动偏移量，即让mScroller的getCurrX()、getCurrY()发生变化
     ...
     * 获得对应日期的农历、节气、节日的描述词
     */
    protected String getCalendarDes(int year,int month,int day){
        Calendar calendar = Calendar.getInstance();
        calendar.set(year,month,day);
        return new CalendarUtil(calendar).toString();
    }
